...
HTIF::HTIF() : exitcode(0), sysbus(0), tohost_addr(0), fromhost_addr(0), syscall(this) {}
...
void HTIF::run()
{
    ...
    if (tohost & 1) { // pass or fail
        ...
        if (exit_code()) { // meaning fail
            std::cerr << "There is error (" << exit_code() << ")" << std::endl;
        }
    }
    else if (tohost) {
        uint64_t sys_id, ret;
...
uint64_t HTIF::exit_code() { return exitcode >> 1; }

bool HTIF::exit() { return exitcode; }
