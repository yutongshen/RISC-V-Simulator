...
int main(int argc, char **argv) {
  ...
  bool done;
  sprintf(main_cmd,
          "%s/main %s/boot.bin %s/main.bin -dump -mem_addr 0x%08x -mem_len "
          "0x1000 -sim_end "
          "0x%08x -sim_end_code 0x%08x -cycle 0xffffffff -o %s/%s",
          argv[1], argv[2], argv[2], TEST_START, SIM_END, SIM_END_CODE, argv[1],
          dump_file);
  printf("%s", main_cmd);
  system(main_cmd);
